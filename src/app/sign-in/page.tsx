...
export default async function SignIn(props: { searchParams: Promise<{ message?: string, error?: string }> }) {
                ...
                <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-xl overflow-hidden relative">
                    ...
                    <div className="p-8">
                        <div className="text-center mb-6">
                            ...
                            <p className="text-slate-400">Sign in to access your portfolio and <span className="text-emerald-400 font-medium">EA files</span></p>
                        </div>

                        {searchParams?.message && (
                            <div className="mb-6 p-4 rounded-lg bg-emerald-500/10 border border-emerald-500/20 flex items-start gap-3">
                                <CheckCircle2 className="h-5 w-5 text-emerald-400 shrink-0 mt-0.5" />
                                <p className="text-sm text-emerald-400 font-medium leading-relaxed">{searchParams.message}</p>
                            </div>
                        )}
